...
func GetInfo(uid string) (info Class.User) {
	...
	_ = tx.QueryRow(sqlStr, uid).Scan(&info.Uid, &info.Password, &info.Email, &info.Create_time)
...
func VerifyPassword(uid string, password string) bool {
	...
	var user Class.User
	_ = tx.QueryRow(sqlStr, uid).Scan(&user.Password)
	tx.Commit()
